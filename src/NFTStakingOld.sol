...
//contract OldNFTStaking is Initializable, OwnableUpgradeable, UUPSUpgradeable, ReentrancyGuardUpgradeable, OwnerManager {
//    string public constant projectName = "DeepLink";
//
//    enum StakingType {
//        ShortTerm,
//        LongTerm,
...
//    }
//
//    StakingType public constant stakingType = StakingType.ShortTerm;
//
...
//        (address machineOwner, uint256 calcPoint, uint256 cpuRate,,,) = dbcAIContract.getMachineInfo(machineId);
//
//        require(cpuRate >= 3500, "cpu rate must be greater than or equal to 3500");
//        require(msg.sender == machineOwner, "sender must be machine owner");
...
//        StakeInfo storage stakeInfo = machineId2StakeInfos[machineId];
//        require(
//            msg.sender == dlcClientWalletAddress || msg.sender == stakeInfo.holder, "not dlc client wallet or owner"
//        );
//        require(stakeInfo.startAtBlockNumber > 0, "staking not found");
//        require(block.number >= stakeInfo.rentEndAt, "staking not ended");
...
//        stakeInfo.nextRenterCanRentAt = block.number + 100;
//
...
//    }
//
//    function getMachineHolderAndCalcPoint(string memory machineId) external view returns (address, uint256) {
//        return (machineId2StakeInfos[machineId].holder, machineId2StakeInfos[machineId].calcPoint);
//    }
//
...
////
////        (bool isOnline, bool isRegistered) = dbcAIContract.getMachineState(machineId, projectName, stakingType);
////        if (!isOnline || !isRegistered) {
////            return false;
////        }
////
////        StakeInfo storage stakeInfo = machineId2StakeInfos[machineId];
////        if (stakeInfo.nextRenterCanRentAt > block.number) {
...
////            StakeInfo storage stakeInfo = machineId2StakeInfos[machineId];
////            if (stakeInfo.calcPoint == 0 && stakeInfo.startAtBlockNumber > 0) {
////                (, uint256 calcPoint,,,,) = dbcAIContract.getMachineInfo(machineId);
////                joinStaking(machineId, calcPoint, stakeInfo.reservedAmount);
////            }
////            emit MachineRegister(machineId, stakeInfo.calcPoint);
////        } else if (tp == NotifyType.MachineUnregister) {
////            StakeInfo storage stakeInfo = machineId2StakeInfos[machineId];
////            if (stakeInfo.startAtBlockNumber > 0) {
////                joinStaking(machineId, 0, stakeInfo.reservedAmount);
////            }
